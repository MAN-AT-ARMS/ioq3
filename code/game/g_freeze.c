	...
	for ( i = 0; i < g_maxclients.integer; i++ ) {
		...
		if ( !self->count ) {
			if ( g_dmflags.integer & 1024 || g_gametype.integer == GT_CTF ) {
				self->count = level.time + ( g_thawTime.integer / 2 );
			} else {
			...
			AddScore( e, self->s.pos.trBase, g_thawBonus.integer );

			e->client->sess.wins++;
			G_Damage( self, NULL, NULL, NULL, NULL, 100000, DAMAGE_NO_PROTECTION, MOD_TELEFRAG );
			Team_ReturnFlagSound( tent, TEAM_RED );
		}
		return;
	}
	self->count = 0;
...
static void Body_think( gentity_t *self ) {
	...
	if ( level.time - self->timestamp > ( g_autothawTime.integer * 1000 ) || ( ( g_dmflags.integer & 1024 || g_gametype.integer == GT_CTF ) && level.time - self->timestamp > ( ( g_autothawTime.integer * 1000 ) / 2 ) ) ) {
		player_free( self->target_ent );
		TossBody( self );
		return;
	}
...
static void CopyToBody( gentity_t *ent ) {
	...
	body->noise_index = G_SoundIndex( "sound/world/lava_short.wav" );
...
void player_freeze( gentity_t *self, gentity_t *attacker, int mod ) {
	...
	if ( self != attacker && g_gametype.integer == GT_CTF ) {
		vec3_t	dist1, dist2;
...
void SpawnWeapon( gclient_t *client ) {
	...
	if ( g_wpflags.integer & 256 ) {
		...
		client->ps.ammo[ WP_NAILGUN ] = 100;
	}
	if ( g_wpflags.integer & 512 ) {
		client->ps.stats[ STAT_WEAPONS ] |= 1 << WP_PROX_LAUNCHER;
		client->ps.ammo[ WP_PROX_LAUNCHER ] = 20;
	}
	if ( g_wpflags.integer & 1024 ) {
		client->ps.stats[ STAT_WEAPONS ] |= 1 << WP_CHAINGUN;
		client->ps.ammo[ WP_CHAINGUN ] = 200;
	}
#endif
