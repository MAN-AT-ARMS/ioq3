...
void G_printMatchInfo( gentity_t *ent ) {
	int i, j, cnt, eff;
	int tot_kills, tot_deaths, tot_gp, tot_sui, tot_tk, tot_dg, tot_dr, tot_acc;
	gclient_t *cl;
	char *ref;
	...
	for ( i = TEAM_RED; i <= TEAM_BLUE; i++ ) {
		...
		CP( "print \"\n^7TEAM   Player          Kll Dth Sui Thw Eff  ^3GP^7    ^2DG    ^1DR   ^6Acc  ^3Score\n"
			"^7-----------------------------------------------------------------------\n\"" );
		...
		for ( j = 0; j < level.numPlayingClients; j++ ) {
			...
			cnt++;
			CP( va( "print \"%-10s %s%-15s^3%4d%4d%4d%4d%s%4d^3%4d^2%6d^1%6d^6%6d^3%7d\n\"",
					aTeams[i],
					ref,
					n2,
					cl->sess.kills,
					...
					ref,
					eff,
					cl->ps.persistant[PERS_SCORE] - cl->sess.kills,
		...
		CP( va( "print \"^7-----------------------------------------------------------------------\n"
				"%-10s ^5%-15s%4d%4d%4d%4d^5%4d^3%4d^2%6d^1%6d^6%6d^3%7d\n\"",
				aTeams[i],
				"Totals",
				tot_kills,
				tot_deaths,
